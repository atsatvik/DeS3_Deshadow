...
def pca(
    ...
    if all_together:
        ...
        split_idxs = np.array(
            [num_patches[0] * num_patches[1] for num_patches in num_patches_list]
        )
    ...
    results = [
        (pil_image, img_pca.reshape((num_patches[0], num_patches[1], n_components)))
        for (pil_image, img_pca, num_patches) in zip(
            image_pil_list, pca_per_image, num_patches_list
        )
    ]
    return results
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root_dir", type=str, required=True, help="The root dir of images."
    )
    parser.add_argument(
        "--save_dir", type=str, required=True, help="The root save dir for results."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=4,
        type=int,
        help="number of pca components to produce.",
    )
    parser.add_argument(
        "--last_components_rgb",
        default="True",
        type=str2bool,
        help="save last components as rgb image.",
    )
    parser.add_argument(
        "--save_resized",
        default="True",
        type=str2bool,
        help="If true save pca in image resolution.",
    )
    parser.add_argument(
        "--all_together",
        default="True",
        type=str2bool,
        help="If true apply pca on all images together.",
    )
    ...
    with torch.no_grad():
        ...
        print("saving images")
        for image_path, (pil_image, pca_image) in tqdm(
            zip(images_paths, pca_per_image)
        ):
