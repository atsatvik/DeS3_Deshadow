...
class SRD:
    ...
    def get_loaders(self, parse_patches=True, validation="SRD"):
        print("=> evaluating SRD test set...")
        train_dataset = ShadowDataset(
            dir=os.path.join(self.config.data.data_dir, "data", "SRD"),
...
class ShadowDataset(torch.utils.data.Dataset):
    ...
    def get_images(self, index):
        ...
        gt_name = self.gt_names[index][:-4] + "_no_shadow.jpg"
        img_id = re.split("/", input_name)[-1][:-4]
        input_img = (
            PIL.Image.open(os.path.join(self.dir, input_name))
            if self.dir
            else PIL.Image.open(input_name)
        )
        try:
            gt_img = (
                PIL.Image.open(os.path.join(self.dir, gt_name))
                if self.dir
                else PIL.Image.open(gt_name)
            )
        except:
            gt_img = (
                PIL.Image.open(os.path.join(self.dir, gt_name)).convert("RGB")
                if self.dir
                else PIL.Image.open(gt_name).convert("RGB")
            )
